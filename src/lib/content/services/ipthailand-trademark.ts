...
export const service: Service = {
	...
	body: [
		...
		},
		{
			type: 'p',
			text: {
				th: 'ลองค้นคำว่า “จดเครื่องหมายการค้า” เป็นภาษาไทย แล้วผลลัพธ์อันดับต้น ๆ มักเป็นสำนักงานตัวแทนและสำนักงานกฎหมายที่ลงทุนกับโฆษณาและการทำอันดับอย่างจริงจัง ส่วนหน่วยงานที่ออกสิทธิให้จริงกลับอยู่ไกลลงไปกว่านั้น ผู้ประกอบการรายเล็กจำนวนมากจึงจ่ายค่าบริการโดยไม่เคยรู้ว่าตนเองยื่นเองได้ ยิ่งการตรวจสอบของเราพบว่าโดเมนหลักที่ไม่มี www ปฏิเสธคำขอทุกชนิดด้วยรหัส 403 และเส้นทางที่ถูกเปลี่ยนไปก็ไม่มีไฟล์ robots.txt ให้อ่าน เครื่องมือทำดัชนีและบริการเก็บถาวรก็ยิ่งตรวจสอบหรือยืนยันเนื้อหาแทนได้ยากขึ้น หน้านี้จึงทำหน้าที่เพียงอย่างเดียว คืออธิบายว่าแต่ละสิทธิคืออะไรด้วยคำที่คนใช้จริง แล้วชี้ไปยังต้นทางที่เป็นทางการ THGov เป็นสารบบอิสระ ไม่มีความเกี่ยวข้องกับกรมทรัพย์สินทางปัญญา ไม่รับยื่นคำขอแทนใคร และไม่เก็บค่าบริการใด ๆ',
				en: 'Search “จดเครื่องหมายการค้า” in Thai and the top results are usually filing agents and law firms with real advertising and ranking budgets, while the office that actually grants the right sits well below them. Plenty of small businesses pay for help without ever learning they may file themselves. And our own check found the bare apex domain refusing every request with a 403 while the path it redirects to serves no robots.txt at all, which makes it harder for indexers and archiving services to verify or vouch for the site’s contents on its behalf. So this page does one thing: explain each right in the words people really use, then point at the official source. THGov is an independent directory with no connection to the department; we file nothing on anyone’s behalf and charge nothing.'
			}
		}
	],
	crawl: {
		...
		verdict: 'none',
		status: 404,
		snippet: 'www.ipthailand.go.th\n  GET /robots.txt  ->  303 redirect -> /th/robots.txt -> 404\n  GET /            ->  303 redirect -> /th/\nipthailand.go.th (apex, no www)\n  GET /robots.txt  ->  200\n  GET /            ->  403 (Googlebot and browser alike)',
		checkedAt: '2026-08-28',
		note: {
			th: 'โดเมนที่มี www เปลี่ยนเส้นทางไปยังเส้นทางภาษา /th/ ซึ่งไม่มีไฟล์ robots.txt (ตอบกลับ 404) ตามมาตรฐานจึงถือว่าเปิดให้เครื่องมือค้นหาเก็บข้อมูลได้ ส่วนโดเมนหลักที่ไม่มี www ปฏิเสธคำขอทุกชนิดจากเครื่องที่เราใช้ตรวจสอบด้วยรหัส 403 ทั้งเมื่อระบุตัวเป็น Googlebot และเมื่อใช้เบราว์เซอร์ทั่วไป ผู้ใช้ที่พิมพ์ชื่อโดเมนโดยไม่มี www จึงอาจเข้าไม่ได้ ทั้งที่เว็บไซต์ยังทำงานปกติ',
			en: 'The www host redirects to the /th/ language path, which serves no robots.txt (404) — by convention that permits crawling. The bare apex domain, however, refuses every request from our audit host with 403, to a Googlebot user-agent and an ordinary browser alike. Anyone typing the domain without the www prefix may therefore be turned away even though the site itself is working.'
		}
	},
	priority: 80,
	updated: '2026-08-28'
